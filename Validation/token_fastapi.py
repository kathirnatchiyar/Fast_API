...
def decryptToken(transDict,sysdbConnection,ruleInformationList):
    try:
        ...
        # print(transDict)
        keyfetchingsql = "SELECT PARAMETER_VALUE FROM OWNER_CONFIG WHERE OWNER_ID="+transDict['custownerid']+" AND PARAMETER_NAME = 'TOKEN::AES::ENCRYPTION/DECRYPTION' AND ACTIVE = 'Y' LIMIT 1"
    ...
    try:
        if(token != None and token !=""):
            ...
            if(privilegeName not in extractedDataPrivileges):
                ...
                if(transDict['custownerid'] != extractedData[0]):
                    errorDict = {}
...
def validateHeaders(rheaders, requestJson, ruleInformationList):
    try:
        ...
        required_inputs = ["custownerid",  "customername", "customertype",  "sessionuser", "lob", "transdb",  "uiux",  "workflow",  "rating",  "reportingdb"]
